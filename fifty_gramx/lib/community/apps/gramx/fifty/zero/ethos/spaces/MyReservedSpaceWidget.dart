...
class MyReservedSpaceWidget extends StatefulWidget {
  const MyReservedSpaceWidget({Key? key})
      : super(key: key);

  @override
  State<MyReservedSpaceWidget> createState() =>
      _MyReservedSpaceWidgetState();
}
...
class _MyReservedSpaceWidgetState extends State<MyReservedSpaceWidget> {
              ...
              Visibility(
                ...
                child: Neumorphic(
                  style: NeumorphicStyle(
                    ...
                    border: NeumorphicBorder(
                        color: AppColors.backgroundStateDisabled(context), width: 2),
                  ),
                  child: Container(
  ...
  updateReservedSpaceDetails() async {
    ...
    if (account.accountId != "") {
      SpaceAccessTokenResponse spaceAccessTokenResponse = await AccessSpaceService.spaceAccessToken();
      if (spaceAccessTokenResponse.spaceServicesAccessDone) {
        setState(() {
          mySpace = spaceAccessTokenResponse.spaceServicesAccessAuthDetails.space;
          myAccount = account;
